...
async def send_u2info_unadd(message: Message, bot: AsyncTeleBot):
    """删除用户信息"""
    message_data = message.text.split(" ")
    if len(message_data) == 2:
        if message.reply_to_message is None:
    ...
    data = sql.inqury_user(tg_id=tg_id, u2_id=u2_id)
    if not bool(data):
        return await bot.reply_to(message, text="无记录 如要删除记录请先添加记录")
    else:
        sql.insert_admin_log(
            message.from_user.id,
            message.from_user.full_name,
        ...
        sql.delete_user(tg_id, u2_id)
        await bot.send_message(message.chat.id, text="已删除")
    return await bot.delete_message(message.chat.id, message_id=message.message_id)